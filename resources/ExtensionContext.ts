 ...
 */
export async function setContextValue(key: string, value: string): Promise<void> {
 ...
 */
export async function getContextValue(key: string): Promise<string | undefined> {
  return context.workspaceState.get(key);
}

export function getGlobalStorageUri(): Uri {
  console.log(context.globalStorageUri.fsPath);
  const customUrl = process.env.TEMP + '\\' + 'globalStorage' + context.globalStorageUri.fsPath.split('globalStorage').at(-1);
  return Uri.file(customUrl);
